  ...
  \begin{itemize}
   \item Natural summaries
    \begin{itemize}
      ...
      \item $p(t)$, the probablility-in-state vector
...
\begin{frame}{Why focus on simplicity}
  \begin{itemize}
    ...
    \item Thoughful simplicity: models which over-summarize are fit in order to 
    ...
    \item $\hat y$ should be unbiased
      \begin{itemize}
        \item since many values will be averaged, 
          bias is more worrysome than variance
...
\begin{frame}
  {\Large Time dependent covariates}
\end{frame}
