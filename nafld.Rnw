...
\begin{frame}{NAFLD}
  \begin{itemize}
    \item Allen, Non-alcoholic fatty liver disease incidence and
...
\begin{frame}{Data}
  \begin{itemize}
    ...
    \item \code{nafld3}: Variables of id, days, and event type.  One observation
      for each outcome: occurrence of NASH, of hypertension, diabetes, etc.
...
\begin{frame}{Data}
  \begin{itemize}
    \item Comorbidities are diabetes, hypertension, and dyslipidemia
...
nfit1 <- coxph(Surv(age1, age2, death) ~ male + nafld,
               data=data1)
nfit2 <- coxph(Surv(age1, age2, death) ~ male + nafld + as.numeric(cstate),
               data=data1)
...
print(multi, digits=2)
@ 
\end{frame}

\begin{frame}
<<mfig3>>=
plot(multi, lty=1:2, col=rep(c(3,5,4,1,2), each=2), yscale=100,
...
mfig2 <- survfit(Surv(age1, age2, endpoint) ~ nafld, data=data2,
                istate=cstate, id=id, se=FALSE, start.time=50)

tmat <- matrix(summary(multi, rtime=100)$table[, "rmean"], 2, 5)
temp <- matrix(summary(mfig2, rtime=100)$table[, "rmean"], 2, 5)
tmat2 <- rbind(Control2 = temp[1,], Std=tmat[1,])
t2 <- tmat2[,c(5,1,2,3)]
t2 <- cbind(t2, Total =rowSums(t2))
colnames(t2) <- c("0mc", "1mc", "2mc", "3mc", "Total")
round(t2, 1)
