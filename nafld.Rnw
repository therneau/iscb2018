...
\begin{frame}{Time in state, based on the fitted model}
  \begin{itemize}
    \item Simplest method is 3.1: time-dependent fit + prediction for static
      ...
      \end{enumerate}
    \item Better is method 1: time-fixed covaraite fit + static prediction
      \begin{enumerate}
        \item Create the doubled data set (full follow-up for controls that
          become cases, as controls)
        \item Cox model fits for each transition, with robust variance
