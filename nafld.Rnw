...
\begin{frame}{Using mstate}
<<mstate2d>>=
pdata <- expand.grid(nafld=0:1, male=0:1, strata=1:10)
pdata$sgrp <- factor(c(1,1,1,4, 2,2,5, 3,6,7)[pdata$strata])
attr(pdata, "trans") <- transitions
class(pdata) <- c("msdata", "data.frame")
mhaz <- msfit(allfit2, newdata=pdata, trans=transitions)
@ 
\end{frame}
