...
\subsection{Transplant waiting list}
...
\begin{frame}{IJ estimator}
  ...
  \begin{itemize}
    ...
    \item For a 2 state alive-dead model, the IJ estimate = Greewood's estimate
       \begin{itemize}
         \item emprically verified for a large number of data sets
...
\begin{frame}{Building data sets}
  \begin{itemize}
     \item Think through the special cases
	  ...
	  \item PAD data set: a joint left/right amuptation
