...
The immediate follow-up question to a non-proportional effect is
...
covariate effect have often been observed to fade with time;
...
of that longer lifetime in the 3 comorbidity state.
Comparing control to NAFLD (left or right pair pair), NAFLD subjects
...
of 0mc:1mc transitions, then the data needed for 1mc:2mc transitions, etc.
