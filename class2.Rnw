...
analyses.  
The primary reference for the study is Allen \cite{Allen2018}.
...
detailed time line information, and treat each subject as if he/she
...
\begin{itemize}
  \item There are \Sexpr{tc[1,7]} deaths, which by definition fall at the
  ...
  \item Conversely, \Sexpr{tc[5,7]} subjects were diagnosed with hypertension
    ...
    occurrence of hypertension event (\code{e2}), but the time-dependent 
