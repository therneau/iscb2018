...
<<mstate4>>=
temp <- msdata[match(1:10, msdata$trans), c('trans', 'male', 'nafld', 'trans2')]
pdata <- expand.grid(nafld=0:1, male=0:1, trans=1:10,
                     trans= c(1,1,1,2,2,6:10))
attr(pdata, "trans") <- transitions
class(pdata) <- c("msdata", "data.frame")

# there is a problem with my mhaz call, not yet addressed: the next line fails
...
The tcount matrix has a few new features.
...
# print out a table comparing these models to prior ones
