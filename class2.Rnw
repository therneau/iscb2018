...
<<sfig1>>=
nafld0 <- with(data1, nafld[match(id, id)])  # nafld status at entry
...
For the latter the simple PH test of a non-zero linear slope was clearly
insufficient.
...
which is what an ordinary \code{coxph} model estimates, is about the same
...
The immediate follow-up question to a non-proprional effect is
``how do I summarize it''?   
There are multiple ways to proceed; a formal one is to use the
\code{timereg} package, which implements the extended Cox model
\begin{equation}
...
It can also have issue if the risk sets get too small, so we truncate the
estimation at age 95.  
<<timereg, fig.caption='Cumulative regression coefficient for females and males'>>=
...
The \code{timecox} function is based on cumulative hazard and cumulative
coefficients; that is the scale on which formal testing can most easily
be done.  For understanding an effect, however, it simpler to work with
the coefficient function itself $\beta(t)$.
...
$tx$. The survSplit routine expands the original data set into 1 year
...
timedata$nafld2 <- with(timedata, nafld *age1)
...
any inference is suspect.  
Many authors suggest using $\beta(t) = \log(t)$ based on the fact that
covaratiate effect have often been observed to fade with time;
however fitting an arbitrary fixed shape without even looking at the data
is an even worse course.  
We might conclude that nothing is going on simply because of a bad choice
for $\beta(t)$, and learn little about the true shape.
A more sophisticated approach is to fit a spline function.

<<slight2>>=
tfit3 <- coxph(Surv(age1, age2, death) ~ nafld + nafld:ns(age1,3), data1,
               subset= (male==0))
anova(cfit2f, tfit3)

tdata <- data.frame(age1=20:90, nafld=1)
plot(20:90, predict(tfit3, tdata),  type='l',
     xlab="Age", ylab="beta(t) for NAFLD")) 
@ 

In summary, one can get a formal test from either the \code{timereg}
function or by anova approach above.  
To describe the shape of the effect requires a plot, or after looking
at the plot one can choose to describe the hazard ratio 
using a simple approximation.
For the NAFLD effect in females the log(HR) is nearly linear 
over the range of 20--100 years.  
...
hazards have a common coefficients, and similarly for the 1:3 and 2:3
transitions.
...
\begin{enumerate}
  \item Fit the model using baseline covariates as predictors, predict for
    a given subject using that subject's baseline.  This is the classic
    ...
    future time point, using the baseline covariates that will apply at that
    instant.
  \item Fit the model using time dependent covariates, and then do prediction
     for a given subject using only their baseline.  This is essentially
     the approach of Simon and Makuch \cite{Simon84}.  The approach is
     commonly used but below I discuss some problems.
   \item Fit the model using time dependent covariates, and do prediction
...
Chronic low level inflammation slowly but inexortably generate scar 
tissue leading eventually to cirrhois and death.
...
<<pbc2, fig.caption='Overlaid curves for tertiles of baseline risk score along with the time=dependent prediction'>>=
...
<<ag1>>=
data1$nafld0 <- with(data1, nafld[match(id, id)])
...
barplot(height= t(rtime[c(2,1,3,4), c(5,1,2,3,4)]), space=c(.05, .5),
        names.arg=c("F NAFLD", "F Control", "M Control", 
                    "M NAFLD"),
        legend= TRUE, args.legend=list(x=10, y=26),
...
of that lifetime in the 3 comorbidity state.
...
An issue with the stacked bar chart is that many will assume that every
subject spends ``after age 40'' time in every state.

Now repeat the curves using fitted Cox models. 
I will first do this ``by hand'' because it is illustritive. 
(Later is will be demonstrated using the mstate package.)
...
as separate fits (yeilding the same coefficients).
...
for the different transtions. 
To do this it requires a ``stacked'' data set:
the first set of rows are the ones that would be used for a Cox model
of 0mc:1mc transtions, then the data needed for 1mc:2mc transitions, etc.
for all of the 10 possible given in our transitions matrix.
Each of the individual subsets is marked with a (from, to, transition) triplet
and contains variables (Tstart, Tstop, status) where status is 1 if the
transition in question occured. 
...
ntrans <- sum(!is.na(transitions))
temp <- vector("list", ntrans)
k <- 0
for (i in 1:5) {
    for (j in 1:5) {
        number <- transitions[i,j]
        if (!is.na(number)) {
            k <- k +1
            temp[[k]] <- stacker(i,j, number)
        }
    }
}
...
a set of rows for each of the transitions, and also needs to
have the transition matrix spliced on.
...
tdata <- expand.grid(nafld=0:1, male=0:1, cstate= levels(data1$cstate))
pfit1 <- survfit(nfit1, newdata=tdata)
@ 
...
We see that adding in the careful timeline data makes almost no change, at least
for this data set.  We will not persue this further.
