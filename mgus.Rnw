...
\begin{frame}{Monoclonal Gammopathy of Undetermined Significance (MGUS)}
  \begin{itemize}
    \item Subjects with a dominant clone in their plasma cell population,
      but without malignancy ($\ge 2$\% of plasma cells).  
...
plot(pfit2, col=1:8, lty=rep(1:2, each=8), mark.time=FALSE,
...
\begin{frame}[fragile]
<<cmugus1, echo=TRUE>>=
cfit1 <- coxph(Surv(etime, event=="death") ~ sex + mspike, mgus2)
round(summary(cfit1)$coef, 2)

cfit2 <- coxph(Surv(etime, event=="PCM") ~ sex + mspike, mgus2)
round(summary(cfit2)$coef, 2)
