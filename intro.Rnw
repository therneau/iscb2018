  ...
  \begin{itemize}
    \item ``time'' as incomplete data
	  ...
	  \item Subjects go from state to state
      	  \item Some have many transitions
       	  \item Some have zero
 ...
 \pause       
     \item Martingale theory gives a formal underpinning.
       \begin{itemize}
           ...
           \item $N_i(t)$ = number of events so far \\
               $Y_i(t)$ = 1 if 'under observation' at time $t$
       \end{itemize}
  \end{itemize}
\end{frame}
...
y3 <- cbind(dlnorm(xx, 1, 1)/plnorm(xx, 1, 1, lower=FALSE),
            dlnorm(xx, 1.5, 1)/ plnorm(xx, 1.5, 1, lower=FALSE))
matplot(xx, y3, type='l', lwd=2, lty=1, log='y', 
        xlim=c(.5, 10), xlab='x', ylab="Hazard")
y4 <- cbind(plnorm(xx, 1, 1, lower=FALSE), plnorm(xx, 1.5, 1, lower=FALSE))
...
\begin{frame}{Informative censoring}
  \begin{itemize}
    ...
    \item You cannot cease following someone because of something that will
      ...
      \begin{itemize}
         \item Analysis of those who "comply with the treatment"
	 \item People drop out because they are about to fail \\
	     MDPIT trial (Oakes et al, JASA 1993; 88:44-49)
	 \item Only those who are sick respond to queries.
	 \pause
	 \item The fact that someone dropped out allows you to better guess
	 their death rate.
     \end{itemize}
  \end{itemize}
\end{frame}
